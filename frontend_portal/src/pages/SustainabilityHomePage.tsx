...
const demoNews: NewsData[] = [
  { id: 1, title: "Sustainability Milestone Achieved", summary: "Our company celebrates a major milestone in sustainability efforts.", image_url: null, created_at: "2025-06-01T10:00:00Z" },
  { id: 2, title: "New Green Initiative", summary: "Introducing our latest eco-friendly initiative to reduce carbon footprint.", image_url: null, created_at: "2025-05-28T12:00:00Z" },
  { id: 3, title: "Community Sustainability Program", summary: "We’re proud to support local communities with our green initiatives.", image_url: null, created_at: "2025-05-20T09:00:00Z" },
];
...
const SustainabilityHomeSlideshow: React.FC = () => {
  ...
  const fetchSustainabilityHomes = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/api/sust/homeSlider", { headers });
      ...
      const message =
        "Failed to fetch sustainability homes: " +
        (err.response?.data?.message || err.message || "Unknown error");
      setError(message);
      toast.error("Error fetching sustainability homes.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[80vh] bg-gradient-to-br from-green-600 to-teal-700">
  ...
  if (error && data.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[80vh] bg-gradient-to-br from-green-600 to-teal-700 p-6">
        ...
        <button
          onClick={fetchSustainabilityHomes}
    ...
    return (
      <div className="flex justify-center items-center min-h-[80vh] bg-gradient-to-br from-green-600 to-teal-700">
        <div className="text-white text-2xl font-semibold flex items-center space-x-3">
          <InformationCircleIcon className="w-8 h-8" />
          <span>No sustainability home entries found.</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="relative min-h-[80vh] w-full overflow-hidden bg-gradient-to-br from-green-600 to-teal-700">
      {[...Array(30)].map((_, index) => (
        <motion.div
          key={index}
          className="absolute w-2 h-2 bg-lime-300 rounded-full opacity-30"
          style={{
  ...
  return (
    <div>
      <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-6 flex items-center">
        <BuildingOffice2Icon className="w-8 h-8 mr-3 text-green-600" />
        Featured Group
      </h2>
      {loading ? (
        ...
        >
          <img
            ...
            loading="lazy"
          />
          <h3 className="text-xl font-bold text-green-600 dark:text-green-400 mb-2">{ftGroup.ft_category}</h3>
          ...
          <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
            <Link
              to="/company/ft-group"
              className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-all duration-300 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
            >
              Learn More
