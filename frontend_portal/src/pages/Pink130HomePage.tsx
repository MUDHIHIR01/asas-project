...
const LeadershipHomeSlideshow: React.FC = () => {
  ...
  const fetchFTPinkHomes = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/api/pink130Sliders", { headers });
      if (Array.isArray(response.data.ft_pink_130_homes)) {
        setData(response.data.ft_pink_130_homes);
      } else {
        throw new Error("Unexpected response format");
      }
    } catch (err: any) {
      const message =
        "Failed to fetch FT Pink homes: " +
        (err.response?.data?.message || err.message || "Unknown error");
      setError(message);
      toast.error("Error fetching FT Pink homes.");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    return (
      <div className="flex justify-center items-center min-h-[80vh] bg-gradient-to-br from-indigo-600 to-purple-700">
        <div className="text-white text-2xl font-semibold flex items-center space-x-3">
          <InformationCircleIcon className="w-8 h-8" />
          <span>No FT Pink home entries found.</span>
        </div>
      </div>
    );
  }
