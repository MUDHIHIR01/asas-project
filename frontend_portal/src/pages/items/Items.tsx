...
const ActionButtons: React.FC<{ itemId: number }> = ({ itemId }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      const response = await axiosInstance.delete(`/api/items/${itemId}`);
      toast.success(response.data.message || 'Item deleted successfully', { position: 'top-right' });
  ...
  return (
    <div className="relative">
      <div className="flex gap-2 items-center">
        <Link to={`/edit-item/${itemId}`} className="p-1 text-blue-500 hover:text-blue-600">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Are you sure you want to delete this item?</h3>
...
const Items: React.FC = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await axiosInstance.get<{ items: ItemData[] }>('/api/items');
  ...
  const columns: Column<ItemData>[] = useMemo(
    () => [
      { Header: '#', accessor: 'count' as any, Cell: ({ row }: { row: Row<ItemData> }) => <span>{row.index + 1}</span>, width: 50 },
      { Header: 'Item Category', accessor: 'item_category' },
      { Header: 'Created At', accessor: 'created_at', Cell: ({ value }: { value: string | undefined }) => formatDate(value) },
      { Header: 'Actions', accessor: 'item_id', Cell: ({ value }: { value: number }) => <ActionButtons itemId={value} /> },
    ],
    []
  );

  const tableInstance = useTable<ItemData>(
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Items Data', 20, 10);
    autoTable(doc, {
      head: [['#', 'Item Category', 'Created At']],
      body: data.map((row, index) => [index + 1, row.item_category, formatDate(row.created_at)]),
    });
    doc.save('items_data.pdf');
  ...
  const handleCreateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const itemCategory = (form.elements.namedItem('item_category') as HTMLInputElement).value;

    if (!itemCategory.trim()) {
      toast.error('Item category is required');
      return;
    }
    if (itemCategory.length > 255) {
      toast.error('Item category must not exceed 255 characters');
      return;
    }

    setLoading(true);
    try {
      const response = await axiosInstance.post('/api/items', { item_category: itemCategory });
      toast.success(response.data.message || 'Item created successfully', { position: 'top-right' });
      setTimeout(() => {
        setShowCreate(false);
    ...
    <div className="tailtable p-4 w-full mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg text-gray-800">Items Management</h2>
          <button
        ...
        <div className="flex flex-col sm:flex-row justify-between mb-6 gap-4 w-full">
          ...
          <div className="flex gap-2">
            <button onClick={exportToPDF} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition shadow-md">Export PDF</button>
            <button onClick={exportToExcel} className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition shadow-md">Export Excel</button>
          </div>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Create Item</h3>
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
