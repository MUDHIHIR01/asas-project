...
import FilterBar from '../components/slots/FilterBar';
...
// New TotalCostCard component
const TotalCostCard = ({ totalCost, selectedAds, filteredAds, handleCheckout }: { 
  totalCost: number; 
  selectedAds: AdSlot[]; 
  filteredAds: AdSlot[]; 
  handleCheckout: () => void;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex flex-col sm:flex-row items-center text-gray-800 dark:text-white gap-4">
          <p className="text-lg font-semibold">
            Total Cost: <span className="text-blue-600">{totalCost.toLocaleString()} Tshs</span>
          </p>
...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchAdSlots = async () => {
      try {
        const response = await axiosInstance.get("/api/ad-slots_v1");
        ...
        if (!Array.isArray(slots) || slots.length === 0) {
          ...
          const updatedSlots = slots.map((slot: AdSlot) => ({
            ...slot,
            rate_unit: "Tshs/Hour",
          }));
          setAdSlots(updatedSlots);
          setFilteredAds(updatedSlots);
  ...
  const handleSelect = (ad: AdSlot) => {
    ...
    if (isSelected) {
      ...
      toast.info("Ad slot removed from cart");
    } else {
      updatedSelectedAds = [...selectedAds, ad];
      toast.success(`Added to cart: ${ad.rate} Tshs/Hour`);
    }
  ...
  const handleCheckout = () => {
    if (selectedAds.length > 0) {
      ...
      navigate('/sign-in', { state: { selectedAds } });
    } else {
      toast.error("Please add at least one ad slot to the cart");
    }
  };
    ...
    return (
      ...
      >
          ...
          <div className="grid grid-cols-2 gap-3 text-sm text-gray-600 dark:text-gray-300">
            ...
            <div className="space-y-2">
              <div className="flex items-center">
                ...
                <span><strong>Placement-Type:</strong> {ad.placement_type}</span>
              </div>
              <div className="flex items-center">
                <PhotoIcon className="w-5 h-5 mr-2 text-yellow-500" />
                <span><strong>Rate:</strong> <span className="text-teal-600 font-medium">{ad.rate} {ad.rate_unit}</span></span>
              </div>
              <div className="flex items-center">
        ...
        <div className="px-5 pb-5">
          ...
          >
            <ShoppingCartIcon className="w-5 h-5 mr-2" />
            {ad.available ? 'Add to Cart' : 'Unavailable'}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <h1 className="text-3xl font-extrabold text-gray-800 dark:text-white mb-6 flex items-center">
        ...
        <p className="text-base text-gray-600 dark:text-gray-400 mb-8">
          Explore available digital ad slots, filter by your needs, and sign in to book.
        </p>
